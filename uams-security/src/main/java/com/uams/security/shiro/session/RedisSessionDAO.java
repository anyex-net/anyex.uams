package com.uams.security.shiro.session;
...
public class RedisSessionDAO extends AbstractSessionDAO implements CustomSessionDAO
{
    ...
    public void update(Session session) throws UnknownSessionException
    {
        ...
        try
        {
            String key = new StringBuffer(SESSION_PREFIX).append(UamsConst.SEPARATOR).append(session.getId()).toString();
            // 设置超期时间
            ...
            if (null != principal)
            {
                ...
                buffer.append(UamsConst.SEPARATOR).append(session.getTimeout());
                buffer.append(UamsConst.SEPARATOR).append(session.getLastAccessTime().getTime());
    ...
    public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession)
    {
        ...
        try
        {
            ...
            for (Map.Entry<String, Object> entry : map.entrySet())
            {
                ...
                {
                    String[] ss = StringUtils.split(String.valueOf(entry.getValue()), UamsConst.SEPARATOR);
                    if (ss != null && ss.length == 3)
                    {
                        SimpleSession session = new SimpleSession();
                        String sessionId = entry.getKey();
                        session.setId(sessionId.substring(sessionId.indexOf(UamsConst.SEPARATOR) + 1, sessionId.length()));
    ...
    protected Session doReadSession(Serializable sessionId)
    {
        ...
        try
        {
            String key = new StringBuffer(SESSION_PREFIX).append(UamsConst.SEPARATOR).append(sessionId).toString();
