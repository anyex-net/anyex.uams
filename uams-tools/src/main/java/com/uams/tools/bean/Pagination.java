...
public class Pagination implements Serializable
{
    ...
    protected Integer         rows             = UamsConst.DEFAULT_PAGE_SIZE;
    
    // 当前是第几页
    @ApiModelProperty(value = "当前页数")
    protected Integer         page             = UamsConst.DEFAULT_CURRENT_PAGE;
    
    // 记录开始位置
    @ApiModelProperty(value = "记录开始位置", hidden = true)
    protected Integer         startIndex       = UamsConst.DEFAULT_START_INDEX;
    
    // 记录结束位置
    @ApiModelProperty(value = "记录结束位置", hidden = true)
    protected Integer         endIndex         = UamsConst.DEFAULT_START_INDEX;
    ...
    public void setRows(Integer rows)
    {
        if (rows < 1)
        {
            this.rows = UamsConst.DEFAULT_PAGE_SIZE;
        }
        else
        {
            this.rows = rows;
    ...
    public void setPage(Integer page)
    {
        if (page < 1)
        {
            this.page = UamsConst.DEFAULT_CURRENT_PAGE;
        }
        else
        {
            this.page = page;
