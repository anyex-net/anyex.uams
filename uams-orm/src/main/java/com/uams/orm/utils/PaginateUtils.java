...
public class PaginateUtils
{
    ...
    public static Pagination getPagination(Pagination page, long totalCount)
    {
        if (page == null) // 首次查询请求
        {
            return getPagination(UamsConst.DEFAULT_PAGE_SIZE, UamsConst.DEFAULT_CURRENT_PAGE, totalCount);
        }
        else
        {
    ...
    {
        if (null == page) page = PaginateUtils.getPagination(default_pageSize, UamsConst.DEFAULT_CURRENT_PAGE, totalCount);
    ...
    {
        if (currentPage <= 0 || totalPage == 0) { return UamsConst.DEFAULT_CURRENT_PAGE; }
    ...
    {
        return pageSize <= 0 ? UamsConst.DEFAULT_PAGE_SIZE : pageSize;
    }
}
