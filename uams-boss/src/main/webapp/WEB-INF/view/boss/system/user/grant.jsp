    ...
    $(function() {
        resourceTree = $('#resourceTree').tree({
            ...
            onLoadSuccess : function(node, data) {
                progressLoad();
                $.post( '${ctx}/system/user/findByUserId', {
                    userId : '${id}'
                }, function(result) {
                    var ids = [];
                    if (result && result.code == 200) {
                        $.each(result.object,function(index,data){
                            ids.push(data.orgId);
                        });
                    }
                    if (ids.length > 0) {
        ...
        $('#grantDataForm').form({
            ...
            success : function(result) {
                ...
                if(result){
                    ...
                    $('#grantDataForm').find('input[name="csrf"]').val(result.csrf);
                }
            }
        });
    });
...
<div id="userGrantLayout" class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'west'" title="系统资源" style="width: 300px; padding: 1px;">
        <div class="well well-small">
            <form:form id="grantDataForm" method="post">
                <input name="userId" type="hidden"  value="${id}" readonly="readonly">
