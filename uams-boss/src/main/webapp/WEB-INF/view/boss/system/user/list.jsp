...
<script type="text/javascript">
            ...
            onLoadSuccess:function (node, data) {
                userDataGrid = $('#userDataGrid').datagrid({
                        ...
                        , {
                            ...
                            formatter: function (value, row, index) {
                                ...
                                if(row.authKey==null || row.authKey=='') {
                                    str += $.formatString('<a href="javascript:void(0)" class="user-easyui-linkbutton-data" data-options="plain:true,iconCls:\'fi-wrench icon-blue\'" onclick="grantUserFun(\'{0}\');" >数据授权</a>', row.id);
                                    str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
                                }
                    ...
                    onLoadSuccess: function (data) {
                        <shiro:hasPermission name="system:setting:user:operator">
                        $('.user-easyui-linkbutton-data').linkbutton({text: '数据授权'});
    ...
    function grantUserFun(id){
        parent.$.modalDialog({
            ...
            width: 500,
            height: 400,
            href: '${ctx}/system/user/grant?id=' + id,
            buttons: [{
            text: '确定',
            handler: function () {
                parent.$.modalDialog.openner_dataGrid = userDataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
                var f = parent.$.modalDialog.handler.find('#grantDataForm');
                f.submit();
            }
        }]
    });}
    ...
    <div data-options="region:'north',border:false" style="height: 30px; overflow: hidden;background-color: #fff">
        <form id="searchUserForm">
            <table style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden; width: auto; float: left;">
                <tr>
                    <th>帐号:</th>
                    <td><input name="userName" placeholder="请输入用户帐号"/></td>
                    <th>姓名:</th>
                    <td><input name="trueName" placeholder="请输入用户姓名"/></td>
                    <td>
